...
def despesas_total(request):
    total = Despesas.objects.all().aggregate(Sum('total'))['total__sum']
    if request.method == "GET":
        return JsonResponse({'total': total})
    
def retorna_lucro(request):
        # Obter o total vendido e converter para Decimal
        total_vendido = Vendas.objects.all().aggregate(total_vendido=Sum('total'))['total_vendido'] or Decimal('0')
        total_vendido = Decimal(total_vendido)
        
        # Obter o total de despesas e converter para Decimal
        total_despesas = Despesas.objects.all().aggregate(total_despesas=Sum('total'))['total_despesas'] or Decimal('0')
        total_despesas = Decimal(total_despesas)
        
        # Calcular o lucro
        lucro = total_vendido - total_despesas
